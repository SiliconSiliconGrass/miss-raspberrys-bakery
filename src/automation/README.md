...
onMounted(() => bridge.connect())          // 自动拨号，失败每秒重试
...
3. **`localhost` 解析到 IPv4 还是 IPv6。** 浏览器自行决定用 `::1` 还是
   ...
   在跑却连不上」。
4. **页面本身是不是用 `https` 或局域网 IP 打开的。** `https` 页面连 `ws://`
   会被浏览器按混合内容拦掉（应该用 `wss://` 或改用 http 打开游戏）；用
   `http://192.168.x.x:5173` 这种局域网地址打开页面时，Chrome 的 Local Network
   Access 策略会拦「私网页面 → 本机回环地址」的请求，改用
   `http://localhost:5173` 打开即可。
